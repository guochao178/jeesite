...
public class SystemService extends BaseService implements InitializingBean {
	...
	public void saveUser(User user) {
		if (StringUtils.isBlank(user.getId())){
			user.preInsert();
			//guochao 20180704 注册用户用，因为上传头像时已将userid生产并写入seesion
			String userId = (String)UserUtils.getCache("CKFINDER_USERID", "");
			if ( !"".equals(userId) ) {
				user.setId(userId);
			}
			//guochao 20180704
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	//注册用获取角色列表
	public List<Role> findRegAllRole(){
		return UserUtils.getRegRoleList();
	}
